...
- openssl-devel
- krb5-devel
- [cmake](https://cmake.org) (>= 3.20) - only required for CMake build mode

On RHEL/CentOS/Rocky Linux:
```bash
sudo yum install openssl-devel krb5-devel
# For CMake build mode:
...
Diskquota supports two build systems: PGXS (Makefile) and CMake.

#### Option 1: Build with Apache Cloudberry (Recommended, PGXS mode)

Diskquota is included in the Apache Cloudberry source tree and will be built automatically with other contrib extensions:
...
Or build only the contrib extensions:

```bash
make -j$(nproc) -C gpcontrib
make install -C gpcontrib
```

Or build diskquota alone:
...
#### Option 2: Standalone Build with PGXS

If you have an existing Apache Cloudberry installation:

```bash
cd <diskquota_src>
# Make sure pg_config is in PATH or set PG_CONFIG
make USE_PGXS=1
make USE_PGXS=1 install
```

#### Option 3: CMake Build (Advanced)

CMake build provides additional features like packaging and build-info generation:
...
If the `cloudberry-env.sh` (for 2.1+ ) or `greenplum_path.sh` has been sourced:
...
```bash
cmake .. -DPG_CONFIG=<cloudberry_installation_dir>/bin/pg_config
```
...
1. The diskquota extension is installed (`make install`) on all nodes (coordinator, standby, and segments)
2. The `shared_preload_libraries` is configured and the cluster is restarted
3. The `diskquota` database exists
4. The `diffutils` package is installed (provides the `diff` command required by pg_regress)

```bash
# Install diffutils if not present (required for regression tests)
# On RHEL/CentOS/Rocky Linux:
